...
microcontroller_address = '192.168.0.106'
...
#--- CODENAME "RADAR" -----
anyBodyThere = '' # may not be in the project

#_______MAIN CODE___________________
...
def start(message):
    ...
    updateData = types.InlineKeyboardButton("♻️Обновить данные",callback_data = "Обновить данные")
    ...
    bot.send_message(message.from_user.id, "✌️Приветствую, я чат-бот, созданный для управления умным домом")
    bot.send_message(message.from_user.id, "Все доступные на данный момент функции будут отображены внизу👇")
    ...
    try:
        if call.data == "Обновить данные":
            ...
            try:
                ...
                anyBodyThere  = ''
                sock = socket.socket()
                sock.connect((microcontroller_address, 3030))
